...
run_kmeans <- function(K) {
  ...
  # Add cluster assignment to the tract data
  clust_dat_t$cluster <- km$cluster
  
  # Return the cluster assignments and summary statistics
  clust_summary <- clust_dat_t %>%
    group_by(cluster) %>%
    summarise(
      mean_pop      = mean(pop, na.rm = TRUE),
...
```{r}
# Cleaned tract-level data for clustering
acs_data <- clust_dat_t  # this already has GEO_ID, county, pop, hh_income, income
...
for (K in 2:10) {
  ...
  # Add cluster membership as a new column named like "cluster_K2", "cluster_K3", etc.
